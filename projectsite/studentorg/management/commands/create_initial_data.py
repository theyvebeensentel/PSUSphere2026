    ...
    def create_organization(self, count):
        ...
        for _ in range(count):
            ...
            Organization.objects.create(
                ...
                college_info=College.objects.order_by('?').first(),
    ...
    def create_students(self, count):
        ...
        for _ in range(count):
            Student.objects.create(
                ...
                program_info=Program.objects.order_by('?').first()
            )
    ...
    def create_membership(self, count):
        ...
        for _ in range(count):
            OrgMember.objects.create(
                student_ref=Student.objects.order_by('?').first(),
                organization_ref=Organization.objects.order_by('?').first(),
                date_joined=fake.date_between(start_date="-2y", end_date="today")
            )
        self.stdout.write(self.style.SUCCESS('Initial data for student organization membership created successfully.'))